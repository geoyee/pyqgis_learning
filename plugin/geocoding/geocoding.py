...
from .utils import read_text, save_shp, geo_coding
...
class geocoding:
    ...
    def initGui(self):
        ...
        self.add_action(
            icon_path,
            text=self.tr(u'geocoding tool bar'),
            callback=self.run,
    ...
    def select_input_file(self):
        filename, _ = QFileDialog.getOpenFileName(
            self.dlg, "选择需要打开的地址文件路径", "", "*.txt")
        self.dlg.edtTxt.setText(filename)


    # 导出shp路径
    def select_output_file(self):
        filename, _ = QFileDialog.getSaveFileName(
            self.dlg, "选择需要保存的矢量文件路径", "", "*.shp")
        self.dlg.edtShp.setText(filename)


    # 处理流程
    def process(self):
        ak = self.dlg.edtKey.text()
        if ak is None:
            print("请输入AK")
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            # 添加事件
            self.dlg.btnTxt.clicked.connect(self.select_input_file)
            self.dlg.btnShp.clicked.connect(self.select_output_file)
        # show the dialog
        self.dlg.show()
